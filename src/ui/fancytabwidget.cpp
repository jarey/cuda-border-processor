...
//Ver método modificado FancyTabWidget::showWidget para comprobar la adaptación del elemento gráfico dentro
//de la aplicación y su integración con los elementos gráficos propios utilizados.
...
#include <./src/ui/utils/stylehelper.h>
	...
	,m_statusBar(0)
{
    ...
    listaPaneles = new panelList;
    mainLayout->addWidget(listaPaneles);
...
void FancyTabWidget::showWidget(int index)
{
    ...
    this->listaPaneles->pagesWidget->setCurrentIndex(this->currentIndex());

    Controlador *controlador = Controlador::Instance();
    Constants *constants = Constants::Instance();

	//Se realiza el cast dinámico del panel actual a la superclase de PanelBase de la que heredan todos
	//los paneles y se obtiene el modo asociado al panel concreto.
	QWidget *widget = this->listaPaneles->pagesWidget->currentWidget();
	PanelBase *panel = dynamic_cast<PanelBase *> (widget);
	if(controlador->getApplicationMode()!= panel->getApplicationMode()){
    controlador->setApplicationMode(panel->getApplicationMode());
    controlador->cleanOriginList();
	}
